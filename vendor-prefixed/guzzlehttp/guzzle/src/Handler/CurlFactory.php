...
namespace Rank_Math_Instant_Indexing\GuzzleHttp\Handler;

use Rank_Math_Instant_Indexing\GuzzleHttp\Exception\ConnectException;
use Rank_Math_Instant_Indexing\GuzzleHttp\Exception\RequestException;
use Rank_Math_Instant_Indexing\GuzzleHttp\Promise as P;
use Rank_Math_Instant_Indexing\GuzzleHttp\Promise\FulfilledPromise;
use Rank_Math_Instant_Indexing\GuzzleHttp\Promise\PromiseInterface;
use Rank_Math_Instant_Indexing\GuzzleHttp\Psr7\LazyOpenStream;
use Rank_Math_Instant_Indexing\GuzzleHttp\TransferStats;
use Rank_Math_Instant_Indexing\GuzzleHttp\Utils;
use Rank_Math_Instant_Indexing\Psr\Http\Message\RequestInterface;
use Rank_Math_Instant_Indexing\Psr\Http\Message\UriInterface;
/**
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function createRejection(EasyHandle $easy, array $ctx): PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \Rank_Math_Instant_Indexing\GuzzleHttp\Psr7\Utils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri): string
    {
        ...
        $redactedUriString = \Rank_Math_Instant_Indexing\GuzzleHttp\Psr7\Utils::redactUserInfo($baseUri)->__toString();
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf): void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \Rank_Math_Instant_Indexing\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \Rank_Math_Instant_Indexing\GuzzleHttp\Psr7\Utils::streamFor($sink);
